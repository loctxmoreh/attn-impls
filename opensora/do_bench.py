...
from common import is_cuda, is_hopper, is_rocm
from flash_impl import flash3_attn
from pt_impl import pt_flash, pt_math, pt_sdpa_cpu, pt_xformers
...
def prepare_input(bs, seq_len, num_heads, head_dim, layout="bshd", device="cuda"):
    if layout == "bshd":
        q = torch.randn(
            bs, seq_len, num_heads, head_dim, device=device, dtype=torch.float16
        )
        k = torch.randn(
            bs, seq_len, num_heads, head_dim, device=device, dtype=torch.float16
        )
        ...
        assert layout == "bhsd"
        q = torch.randn(
            bs, num_heads, seq_len, head_dim, device=device, dtype=torch.float16
        )
        k = torch.randn(
            bs, num_heads, seq_len, head_dim, device=device, dtype=torch.float16
        )
...
def benchmark(
    ...
    WARM_UP = 25
    REP = 100
    query, key, value = prepare_input(
        batch_size, sequence_length, num_heads, head_dim, layout=layout, device=device
    )

    ms = triton.testing.do_bench(
        partial(func, query, key, value), warmup=WARM_UP, rep=REP, return_mode="mean"
    )
    tflops = calculate_tflops(
        ms * 1e-3, batch_size, sequence_length, num_heads, head_dim
    )
